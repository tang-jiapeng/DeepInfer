...
void Tensor<float>::set_data(const arma::fcube& data) {
  CHECK(data_.n_rows == this->data_.n_rows) << data_.n_rows << " != " << this->data_.n_rows;
  CHECK(data_.n_cols == this->data_.n_cols) << data_.n_cols << " != " << this->data_.n_cols;
  CHECK(data_.n_slices == this->data_.n_slices) << data_.n_slices << " != " << this->data_.n_slices;
  this->data_ = data;
}
...
void Tensor<float>::Padding(const std::vector<uint32_t>& pads, float padding_value) {
  ...
  uint32_t pad_row1 = pads.at(0);
  uint32_t pad_row2 = pads.at(1);
  uint32_t pad_col1 = pads.at(2);
  uint32_t pad_col2 = pads.at(3);

  arma::fcube new_data(this->data_.n_rows + pad_row1 + pad_row2,
                       this->data_.n_cols + pad_col1 + pad_col2, this->data_.n_slices);
  new_data.fill(padding_value);
  new_data.subcube(pad_row1, pad_col1, 0, new_data.n_rows - pad_row2 - 1,
                   new_data.n_cols - pad_col2 - 1, new_data.n_slices - 1) = this->data_;
...
void Tensor<float>::Flatten(bool row_major) {
  ...
  uint32_t size = this->data_.size();
  this->data_.reshape(1, size, 1);
  this->raw_shapes_.clear();
  this->raw_shapes_.push_back(size);
  this->Fill(values, row_major);
}
